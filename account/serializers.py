...
class ForgotPasswordSerializer(serializers.Serializer):  #требует email
...
class ForgotPasswordCompleteSerializer(serializers.Serializer):  #требует email, password, 
    password = serializers.CharField(required=True, min_length=6)
    password_confirm = serializers.CharField(required=True, min_length=6)
    code = serializers.CharField(required=True)

    def validate(self, attrs):    # чтобы проверить 2 кода используем validate
        p1 = attrs.get('password')
    ...
    def set_new_password(self):
        user = User.objects.get(activation_code=self.validated_data.get('code'))  # 1get - метод object, 2get - method dict
        password = self.validated_data.get('password')
        user.set_password(password)
        user.activation_code = '' # измененные заносит в базу данных
        user.save(update_fields=['password', 'activation_code'])  # т.е password & activ_code